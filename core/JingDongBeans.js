...
function BeanCollector () {
  ...
  this.doDailyTasks = function () {
    // 打开抽屉
    if (this.openTaskDrawer()) {
      // 执行逛一逛任务
      this.pushLog('查找是否存在 去逛逛 任务')
      let taskBtns = widgetUtils.widgetGetAll('去逛逛')
      let executed = false
      if (taskBtns && taskBtns.length > 0) {
        executed |= this.findFirstAndDoTask(taskBtns)
      }
      // 双签任务
      this.pushLog('查找是否存在 去签到 任务')
      let signBtn = widgetUtils.widgetGetOne('去签到', 3000)
      if (signBtn) {
        this.doDoubleSign(signBtn)
      }
      // 去关注频道
      this.pushLog('查找关注频道任务')
      executed != checkFollowChannel.apply(this)
      if (executed) {
        return this.doDailyTasks()
      } else {
        this.backAndCollectAllReward()
        return true
      }
    } else {
      this.pushErrorLog('打开任务抽屉失败')
    }
    // 读取任务列表并执行操作
    return false
  }
    ...
    if (collectableBalls && collectableBalls.length > 0) {
      ...
      balls = collectableBalls.map(target => {
        ...
        if (childCount == 3) {
          ...
          canCollect = /\+\d+/.test(rewardText) && rewardText != '+0'
  ...
  this.backToTaskDrawer = function () {
    ...
    while (!(success = widgetUtils.widgetCheck('.*做任务 得更多收获值.*', 2000)) && limit-- > 0) {
      this.replaceLastLog('返回任务抽屉' + limit)
      automator.back()
      sleep(1000)
    }
    if (!success) {
  ...
  this.openTaskDrawer = function () {
    ...
    if (this.captureAndCheckByOcr('瓜分更多京豆', null, null, null, true)) {
  ...
  this.doubleSign = function (recheck) {
    ...
    this.openTaskDrawer()
    let doubleSignTitle = widgetUtils.widgetGetOne('.*双签.*')
    if (doubleSignTitle) {
      this.doDoubleSign(doubleSignTitle)
    } else {
      this.pushErrorLog('未能找到双签任务入口')
    }
    // TODO 双签领豆 变更入口和处理逻辑
  }
  ...
  function DrugSigner () {
    ...
    this.checkDailySign = function (recheck) {
      ...
      if (parentThis.captureAndCheckByOcr('签到奖励', '签到奖励', null, null, true)) {
        ...
        sleep(1000)
        // 可能存在弹窗，二次校验是否存在入口
        parentThis.captureAndCheckByOcr('签到奖励', '签到奖励', null, null, true)
