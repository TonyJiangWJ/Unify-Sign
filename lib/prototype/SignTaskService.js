...
let SignTaskExecuteLog = {
  tableName: 'SIGN_TASK_EXECUTE_LOG',
  tableCreate: ''
}
...
function SignTaskService () {
   ...
   * 查询任务和分组的关联关系
   * 
   * @param {Number} configId 任务配置id
   * @returns 
   */
  this.queryTaskGroupRef = function (configId) {
    return this.taskGroupRefsDao.selectOne('WHERE CONFIG_ID=?', [configId])
  }
  ...
  this.removeTaskGroupRef = function (groupCode, configId) {
    ...
    logUtils.warnInfo(['groupCode: {} configId: {} 关联关系不存在', groupCode, configId])
    return 0
  }
